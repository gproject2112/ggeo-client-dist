...
var Device = {
    ...
    scan: async function() {
        ...
        try {
            ...
            if (hint) {
                if (scanJson.transport_error) {
                    hint.textContent = self._t(
                        "err_" + scanJson.transport_error,
                        "Apple device service is not running. Check iTunes/Apple Devices (Windows) or usbmuxd (Mac).");
                    hint.hidden = false;
                } else if (silentDevices.length > 0) {
